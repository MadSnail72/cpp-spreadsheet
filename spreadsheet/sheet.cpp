...
const Cell* Sheet::findCellRaw(Position pos) const noexcept {
    const auto it = cells_.find(pos);
    if (it == cells_.end()) return nullptr;
    return it->second.get();
}

Cell* Sheet::findCellRaw(Position pos) noexcept {
    return const_cast<Cell*>(static_cast<const Sheet&>(*this).findCellRaw(pos));
}

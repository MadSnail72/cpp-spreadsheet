...
bool Cell::WouldIntroduceCircularDependency(const Impl& new_impl) const {
    ...
    for (const auto& pos : new_impl.GetReferencedCells()) {
        referenced.insert(sheet_.findCellRaw(pos));
    }
...
void Cell::InvalidateCacheRecursive(bool force) {
    ...
    InvalidateCacheRecursive(visited, force);
}

void Cell::InvalidateCacheRecursive(std::unordered_set<Cell*>& visited, bool force) {
    ...
    for (Cell* incoming : l_nodes_) {
        if (incoming) incoming->InvalidateCacheRecursive(visited);
    }
}
...
void Cell::Set(std::string text) {
    ...
    // Fast path: identical content -> no changes (O(1))
    if (text == impl_->GetText()) return;

    if (text.empty()) impl = std::make_unique<EmptyImpl>();
    else if (text.size() > 1 && text[0] == FORMULA_SIGN) impl = std::make_unique<FormulaImpl>(text, sheet_);
    else impl = std::make_unique<TextImpl>(text);
    ...
    for (const auto& pos : impl_->GetReferencedCells()) {
        Cell* outgoing = sheet_.findCellRaw(pos);
        if (!outgoing) {
            sheet_.SetCell(pos, "");
            outgoing = sheet_.findCellRaw(pos);
        }
...
void Cell::Clear() {
    // Remove edges to referenced cells
    for (Cell* outgoing : r_nodes_) {
        if (outgoing) {
            outgoing->l_nodes_.erase(this);
        }
    ...
    // Set to empty and invalidate dependents' caches
    impl_ = std::make_unique<EmptyImpl>();
    InvalidateCacheRecursive(true);
}
